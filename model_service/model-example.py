...
model = AutoModelForSequenceClassification.from_pretrained(model_name).to(device)  # Move model to GPU if available
...
def batched_predict_sentiment(texts, batch_size=16):
    results = []
...
# batched_predict_sentiment(texts, batch_size=16)
for text, sentiment in zip(texts, batched_predict_sentiment(texts, batch_size=16)):
    print(f"Text: {text}\nSentiment: {sentiment}\n")
